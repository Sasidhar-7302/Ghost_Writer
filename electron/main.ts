...
const logFile = path.join(app.getPath('documents'), 'ghost_writer_debug.log');
...
export class AppState {
  ...
  public async quitAndInstallUpdate(): Promise<void> {
    ...
    if (process.platform === 'darwin') {
      try {
        // Get the downloaded update file path (e.g., .../Ghost-Writer-1.0.9-mac.zip)
  ...
  public setUndetectable(state: boolean): void {
    ...
    if (process.platform === 'darwin') {
      ...
      if (state) {
        ...
        // Revert to "Ghost Writer" appearance when exiting undetectable mode
