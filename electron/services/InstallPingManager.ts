 ...
 * - "app": "ghost_writer" (hardcoded app identifier)
 ...
 * Configurable via GHOST_WRITER_PING_URL env var.
 * Set to empty string or omit to disable install pings entirely.
 */
...
export async function sendAnonymousInstallPing(): Promise<void> {
    try {
        // Early exit if no endpoint configured
        if (!INSTALL_PING_URL) {
            console.log('[InstallPingManager] No ping endpoint configured, skipping');
            return;
        }
