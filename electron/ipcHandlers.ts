...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  ipcMain.handle("save-resume-text", async (event, text: string) => {
    await appState.contextManager.saveResumeText(text);
    return { success: true };
  });

  ipcMain.handle("save-jd-text", async (event, text: string) => {
    await appState.contextManager.saveJDText(text);
    return { success: true };
  });

  ipcMain.handle("upload-resume", async (event, filePath: string) => {
    const text = await appState.contextManager.processFile(filePath, 'resume');
    return { success: true, text };
  });

  ipcMain.handle("upload-jd", async (event, filePath: string) => {
    const text = await appState.contextManager.processFile(filePath, 'jd');
    return { success: true, text };
  });

  ipcMain.handle("get-context-documents", async () => {
  ...
  ipcMain.handle("set-nvidia-api-key", async (_, apiKey: string) => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
      CredentialsManager.getInstance().setNvidiaApiKey(apiKey);

      const llmHelper = appState.processingHelper.getLLMHelper();
      llmHelper.setNvidiaApiKey(apiKey);
  ...
  ipcMain.handle("set-deepseek-api-key", async (_, apiKey: string) => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
