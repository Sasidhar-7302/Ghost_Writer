      ...
      "actionItems": ["specific next steps, assigned tasks, or implied follow-ups. If absolutely none found, return empty array"]
                ...
                if (generatedSummary) {
                    ...
                    try {
                        ...
                        console.log(`[IntelligenceManager] Summary generated: overview=${!!parsed.overview}, keyPoints=${(parsed.keyPoints || []).length}, actionItems=${(parsed.actionItems || []).length}`);
                    } catch (e) {
                        console.error("Failed to parse summary JSON, using raw text as overview", e);
                        // Fallback: store raw LLM text as overview so summary is never empty
                        summaryData = {
                            overview: generatedSummary.substring(0, 2000),
                            keyPoints: [],
                            actionItems: []
                        };
                    }
                } else {
                    console.warn('[IntelligenceManager] LLM returned empty summary');
                }
            } else {
