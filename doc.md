# Reverse-Engineering "Invisible Cheating App" - Ghost Writer Architecture

Everyone saw the viral stunt with "Ghost Writer," the invisible app designed to secretly ace coding interviews. I tore apart the app to see how it works, and turns out, the tech itself is genuinely interesting.

![Ghost Writer Screenshot](image.png)

### How Ghost Writer Actually Works (Technical Breakdown)

Ghost Writer was built using Electron, a desktop app framework based on Chromium and Node.js, to create a transparent, always-on-top overlay:
...
Unlike basic screen wrappers, Ghost Writer uses a **Rust-based native module** (via N-API) to capture system audio with near-zero latency. This stream is processed in real-time, fed to Google's Speech-to-Text API, and converted into a "Rolling Transcript" that the AI constantly analyzes.
...
- **Security Annoyances**: Windows can detect and restrict invisible overlays in secure or fullscreen contexts.
